...
class LoRaWorker:
    def __init__(self, verbose=False, callback=None):118
    ...
    def _simulation_loop(self):
        while self.running:
            ...
            print(
                f"[MOCK UPLINK] Node {dev_id} sending data (Auto: {fake_data['auto_mode']})")
    ...
    def send_command(self, device_id, command_key, value):
        ...
        if device_id in self.simulated_nodes:
            if command_key == "AUTO":
                ...
                # COLOR: 2=Yellow(True), 1=White(False)
                # Sửa logic so sánh cụ thể thay vì bool()
                self.simulated_nodes[device_id]["yellow_color"] = True if value == 2 else False
